...
public class JobExecutorServiceImpl implements IJobExecutorService {
    ...
    public Result<Void> jobDelete(Integer userId, Long jobInstanceId) {
        ...
        JobInstance jobInstance = jobInstanceDao.getJobInstance(jobInstanceId);
        if (jobInstance == null) {
            return Result.failure(SeatunnelErrorEnum.valueOf("Job instance not found"));
        }

        // Get the job status from the engine
        String jobStatus = getJobStatusFromEngine(jobInstance, jobInstance.getJobEngineId());
        if (jobStatus != null && (jobStatus.equals("RUNNING") || jobStatus.equals("PAUSED"))) {
            // If the job is running or paused, cancel it in the engine
            SeaTunnelEngineProxy.getInstance().cancelJob(jobInstance.getJobEngineId());
        }

        // Remove the job instance from the database
        jobInstanceDao.delete(jobInstanceId);

        return Result.success();
    }

    private void stopJobInEngine(String jobEngineId) {}
}
