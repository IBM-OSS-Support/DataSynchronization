      ...
      useTableOperation(
        {
          ...
          buttons: [
            ...
            },
            {
              ...
              onPositiveClick: (row) => { void handleDel(row.id) }, // Use onCallback here
  ...
  const handleDel = (id: number) => {
    hanldleDelJob(id)
      .then(() => {
        ...
        variables.tableData = variables.tableData.filter((item: any) => item.id !== id);
        message.success(t('common.success_tips'));
      })
      .catch((error) => {
        console.error('Error deleting row:', error);
        message.error(t('common.error_tips') || 'An error occurred'); // Default message if translation key is missing
      });
  };

  function onCallback(id: any, action: string) {
    if (action === 'delete') {
      handleDel(id); // Call the handleDel function to delete the item
    } else {
